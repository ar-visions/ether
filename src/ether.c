...
void function_init(function fn) {
    ...
    LLVMSetLinkage(fn->value,
        fn->completer ? LLVMInternalLinkage : LLVMExternalLinkage);
...
void record_completer(record rec) {
    ether e = rec->mod;
    int total = 0;
    record r = rec;
...
void member_init(member mem) {
    ...
    if (!instanceof(top, non_registered)) {
